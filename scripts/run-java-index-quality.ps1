...
try {
    ...
    Invoke-CargoChecked -Arguments @("test", "-p", "opticcode-tools", "java_index")
    Invoke-CargoChecked -Arguments @(
        "test", "-p", "opticcode-cli", "--test", "java_index_cli"
    )
    ...
    Write-Host "CODE-001B1 Java index quality checks passed."
}
finally {
    Pop-Location
}
