...
try {
    ...
    Invoke-CargoChecked -Arguments @("test", "-p", "opticcode-tools", "java_syntax")
    Invoke-CargoChecked -Arguments @(
        "test", "-p", "opticcode-cli", "--test", "java_syntax_cli"
    )
    ...
    Write-Host "CODE-001 Java syntax quality checks passed."
}
finally {
    Pop-Location
}
